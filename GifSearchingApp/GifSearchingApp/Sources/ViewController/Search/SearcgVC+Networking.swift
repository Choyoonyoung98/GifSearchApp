...
extension SearchVC {
    ...
    func getTrendList(keyword: String, offset: Int) {
        let params: [String: Any] = [
             "api_key": "EOxjg9RPBbUJQyzL8yag8Z8u6tN6zauH",
               "limit": 24,
               "offset": offset
        ...
        GetGifTrendService.sharedInstance.getTrendList(params: params) { (result) in
            switch result {
                case .networkSuccess(let data): //200
                    let gifData = data as? GifTrendModel
                    if let resResult = gifData {
                        //TODO: 데이터 뷰에 뿌려주기
                        if let resultData = resResult.data {
                            for res in resultData {
                                let gifTitle = self.gsno(res.title)
                                let gifId = self.gsno(res.id)
                                if let gifURL: URL = URL(string: self.gsno(res.images?.fixed_width_small_still?.url)) {
                                    self.gifDataList.append(GifInfo(title: gifTitle, url: gifURL, id: gifId))
                                }
                            }
                            for a in self.gifDataList {
